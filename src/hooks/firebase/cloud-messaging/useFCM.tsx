...
import { FCM_ENDPOINT } from "@/services/notification/post/endpoints";
...
const useFcmToken = () => {
  ...
  const { auth, setFcm } = useAuthStore();
  ...
  const loadToken = async () => {
    ...
    const isTokenSend = localStorage.getItem("isFcmSend") === "yes";
    const userId = auth?.user?._id;

    if (token && !isTokenSend && userId) {
      axiosInstance
        .post(`${FCM_ENDPOINT}/${userId}`, {
          fcmToken: token,
          platform: "WEB",
        })
        .then(() => {
